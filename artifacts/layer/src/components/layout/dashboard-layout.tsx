...
export function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const navLinks = isCreator ? [
    ...
    { name: "All Videos", href: "/dashboard/creator?tab=all", icon: Video },
    { name: "My Editors", href: "#", icon: Users },
  ] : [
    { name: "Dashboard", href: "/dashboard/editor", icon: LayoutDashboard },
    { name: "My Submissions", href: "/dashboard/editor?tab=all", icon: Video },
  ];
      ...
      <aside className="hidden md:flex flex-col w-64 bg-sidebar border-r border-sidebar-border h-full relative z-20">
        ...
        <div className="px-4 py-2">
          ...
          <nav className="space-y-1">
            {navLinks.map((link) => {
              const isActive = location === link.href || (link.href !== '#' && location.startsWith(link.href) && link.href.includes('tab'));
              return (
