...
export function ScreenshotCarousel() {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto select-none">
      {/* Browser chrome */}
      <div className="rounded-[var(--radius-6)] overflow-hidden shadow-[var(--shadow-4)] shadow-primary/10 border border-border">
        {/* Title bar */}
        <div className="bg-muted border-b border-border px-4 py-3 flex items-center gap-3">
          ...
          <div className="flex-1 flex justify-center">
            <div className="bg-white border border-border rounded-[var(--radius-3)] px-4 py-1 text-xs text-muted-foreground/70 w-52 text-center">
      ...
      <div className="mt-5 flex flex-col items-center gap-3">
        <p className="text-sm text-muted-foreground font-medium">{SLIDES[current].caption}</p>
          ...
          {SLIDES.map((_, i) => (
            <button
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                i === current ? "bg-primary w-8" : "bg-gray-300 w-1.5"
              }`}
            />
          ))}
        </div>
